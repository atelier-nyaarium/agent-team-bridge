...
**P4 Android local build (verified):** the app IS locally buildable. `export JAVA_HOME=/home/nyaarium/android-dev/jdk` (java is NOT on PATH; SDK at `~/android-dev/sdk`, `android/local.properties` points there), then from `android/`: `./gradlew :app:testDebugUnitTest --no-daemon --console=plain` runs the Kotlin unit tests incl. the cross-language golden vectors (SessionIdVectorsTest / ProtocolFixturesTest shared with vitest). Gradle 9.5.1, ~7s cached. So P4 is verifiable locally with the same rigor as the server bun lint+test.

**Final-stretch directive (human, 2026-06-27):** "all the hard foundations are over. do a final questionnaire ... Then push implementation all the way through no stopping." So after the consolidating questionnaire, drive P4 -> P5 -> P6 autonomously, committing green, WITHOUT per-phase verification stops (each still runs its own refinement + audited-implementation, but no checkpoint-and-wait between phases).

**Final questionnaire answers (human, 2026-06-27):**
- **Q-F1 working-chip poll = HYBRID.** The currently-OPEN chat gets an EXPENSIVE continuous poll (always checks idle while you are on it). Every OTHER (background) session gets a CHEAP poll that does NOT rearm - a one-shot idle check, no perpetual loop. So the daemon never continuously polls more than the one open session.
- **Q-F2 crosstalk "this chat here" = via `crosstalk_discover`.** Enhance `crosstalk_discover` to list SESSIONS (not just teams) with a new RECENCY column ("active 5 minutes ago"). An agent finds + addresses the target session from that list. Needs a per-session lastActive timestamp.
- **Q-F3 host-spawn gating = existing console owner-auth.** No extra confirm.

**Implementation defaults (no further fork; surfaced for objection):** Forget drops the resume-map entry AND kills the running tmux (a new kill-session host-op) = full teardown (Q2). Crosstalk MISS auto-creates the named session (wake + spawn, Q5). Asleep "available" sessions surface from the durable resume map (it doubles as the known-session list; Forget removes the entry -> the session leaves the list). The session-name dialog validates a slug (isTmuxName). Recency from a per-session lastActive timestamp, shown relative.

**Loop status:** P0 done. **P1/P2/P3 DONE + sealed.** **P4 DONE + sealed** (cross-language twin + sessionName vectors; available-session list from the resume map; `killSession`+`forget` teardown; peek-backed working chip; spawn-point board with the session-name dialog + nested sessions; catalog-membership classification fixing the dotted-name class; both runtimes green - TS 739, Android `:app:testDebugUnitTest`). **P5 DONE + sealed** (synthetic host spawn-point on the local gateway; Android compile green; verify pass clean). **P6 DONE + sealed** (crosstalk session-addressing: `crosstalk_discover` lists sessions + recency + hides spawn-points; `crosstalk_send` targets `project.session`, MISS wakes/creates via the existing path; TS 742 tests). **PLAN COMPLETE - all phases P0-P6 shipped and sealed on main.**

**Reassessment for P5 (after P4) - host as a symmetric spawn point (owner-gated):** the heavy lifting is ALREADY in place - `create_session` resolves a `host` target, `buildLaunchCommand` has the host form (`export PROJECT_NAME=host.<session>; ...; exec bash`), and `tmuxCore` drives a bare-`tmux` host session. So a daemon-spawned host session registers as the composite `host.<session>` (loose) and the board already nests it. P5 is mostly the APP: surface a **synthetic "host" spawn-point header** (the host is not in `offlineCatalog`, which holds devcontainer dirs) whose dialog runs `create_session(target:"host", session)`; gate it behind the existing console owner-auth (Q-F3 = no extra confirm). Small server check: confirm a host composite session reattaches/forgets the same as a devcontainer one (the host target path through `ensureSession`/`killSession`).

**Reassessment for P6 (after P4) - crosstalk session-addressing + 3-way relay:** `crosstalk_discover` (`mcp/bridge/bridgeDiscover.ts`) grows a **session list + a recency column** ("active 5m ago", from `TeamInfo.lastActive`); `crosstalk_send` (`bridgeSend.ts`) targets a `project.session` - a MISS auto-creates the session (reuse the wake/`create_session` path), a HIT routes to it; the 3-way "talk to this chat here" works because an agent finds the target session in the discover list. Needs `lastActive` to update on activity (the P4 crust note), not just register, for the recency to be meaningful.

**Reassessment for P4 (after P3) - the app phase (Android/Kotlin):** the server side is now fully composite-aware. P4 is the app UX: collapsible spawn-point group headers (devcontainer projects); tap header -> session-name dialog -> spawn a `project.session`; nested session children; the `restored`/`started fresh` chip; the **"Check terminal" chip** fallback for unclean states; the **"working" chip** = a real armed-idle poll (the daemon polls the session's pane via the shared `isAgentReady` and clears on idle); expand terminal-eligibility from `kind=='devcontainer'` to any local daemon-drivable session (`project.session` + host); rename rules. **Cross-language surface lands here** (deferred from P1): the Kotlin twin of `parseSessionName`/`composeSessionName`/`isComposite` + the shared `vectors.json` + `codegen-kotlin.ts` emit of `SESSION_SEP`/`DEFAULT_SESSION`, since the app now constructs/splits composites. Clean break: the app stops treating a bare `project` as a chat (tapping it opens the dialog, not a thread); existing bare threads go stale.
...
- **P4 - App UX (devcontainer + host view).** Collapsible spawn-point group headers; tap header -> session-name dialog (new only; existing name opens the existing child); session children nested; `restored`/`started fresh` chip; expand terminal-eligibility from `kind=='devcontainer'` to **any local daemon-drivable session** (`project/session` + host sessions); update rename rules. **"Check terminal" chip (human directive):** any unclean restore edge case (dead pane, ambiguous state, failed resume) chips the session "Check terminal"; tapping it defaults to the Terminal view. This is the catch-all fallback, so the backend does NOT need perfect restore-state classification - anything not cleanly resumed/fresh routes the user to look. **"working" chip accuracy (human directive):** today the working/busy chip appears to be just "armed until a response arrives" (a guess). Make it real - while a session is armed (working), the host daemon periodically polls that session's tmux for an **idle marker** and clears the chip when the agent goes idle (not only when a reply lands). This reuses the **idle-marker detection P3 factors out** (`❯` / `? for shortcuts` / not-busy, alongside `Claude Code v`). Note this is a SECOND, low-frequency poll DISTINCT from the terminal-view capture poll - it refines Q3's "poll only on the Terminal view" constraint: terminal view = fast capture poll; armed-working = slow idle-check poll. *Verify:* full tap-to-spawn-to-chat-to-terminal loop on device + the working chip clears on real idle.
...
## P3 - detailed design (refinement lap 1)

**Goal:** make devcontainer sessions dynamic, composite-named loose peers - composite registration (which unblocks the composite wake the P2 audit flagged), the durable `project.session -> claudeSessionId` map + `--resume`, the handleWake->docker-exec swap, the resume readiness tweak, and demoting the fixed session. The biggest behavioral phase.

**Identity pinning - the keystone (grounded in code + live evie-bot).** `PROJECT_NAME` is baked into the container IMAGE (`.devcontainer/Dockerfile ENV PROJECT_NAME=$PROJECT_NAME`, build arg from `compose.yml`), inherited by every container process; `mcp/index.ts` uses it verbatim as the team name. So a session registers under whatever `PROJECT_NAME` its process sees. **Mechanism: the launch overrides it per session** - `buildLaunchCommand` prefixes `PROJECT_NAME=<composeSessionName(project,session)> ` on the `claude` invocation (the image ENV is the default; a per-session launch overrides it). The in-container MCP then registers as `project.session`, so `wakeCoordinator.notify("project.session")` fires and the composite wake completes + `/send` to `project.session` resolves (the exact P2 blocker, now unblocked).

**Composite wake de-hardcode (now reachable).** `doWakeTeam(team)` -> `parseSessionName` -> wake that `(project, session)`; `handleWake` (migrated to `tmuxCore` docker exec per the live-test verdict; `ensureContainerUpAsync` stays on the devcontainer CLI) ensures the `<session>` tmux running `PROJECT_NAME=<composite> claude [--resume <id>]`.

**Durable resume map + `--resume`.** `registerMsg` gains `claudeSessionId: CLAUDE_CODE_SESSION_ID` (the MCP already reads this id; just report it). The gateway persists `project.session -> {claudeSessionId, lastSeen}` in a `DurableStore`. `openSession`/`handleWake`: reattach if the tmux session is alive; else if a `claudeSessionId` is mapped, launch `claude --resume <id>` (continue in place, NOT `--fork-session`); else fresh. **Readiness tweak:** a resumed pane shows the restored conversation + idle `❯` + `? for shortcuts`, NOT the `Claude Code v` header - so the idle/ready marker must accept that shape too (the `captureOk` dead-launch signal is unaffected). **Factor this idle-marker check into a reusable helper** - the P4 "working" chip poll (periodic armed-idle check) reuses the same idle detection.

**Forget = teardown (Q2).** Forget drops the durable map entry AND kills the in-container tmux session (stop listing + tear down).

**Demote the fixed session to loose - the open semantics (fork #2 below).** Today a container auto-registers ONE bare `project` session (image `PROJECT_NAME`). "Demote" means the bare auto-session is no longer privileged; sessions are dynamic composite peers. HOW FAR is a human fork (keep a default session for back-compat vs spawn-point-only).

**`/questionaire` forks (P3 needs the human - will ask after the audit validates them):**
1. **DATA_DIR.** The map's home: keep it under `/app/log` with the other durable stores (status quo, deepens the "clear-logs-wipes-federation-keys" landmine) OR introduce a `DATA_DIR` independent of the debug-log filename and migrate ALL durable stores (pending-jobs, mailboxes, replay-guard, federation keys, the new map) - bigger blast radius (docker volume + env), fixes the landmine.
2. **Demote-to-loose default-session semantics.** (a) Keep a default session so a bare `project` still works (e.g. bare maps to a `project.main` or the legacy `claude`), preserving existing chats + a one-tap "just talk to this container"; or (b) fully spawn-point-only - no default, every session must be named, the bare-container chat concept goes away (cleaner model, but migrates/breaks existing bare `project` threads). Affects the app (P4), the catalog, and back-compat.

**Out of P3:** the app UX (spawn headers, name dialog, chip, "Check terminal" fallback) - that's P4. P3 is server/daemon/MCP-side: identity pinning, composite wake, the map + resume, the exec swap, demote.
...
## P4 - detailed design (refinement lap 1)

**Goal:** the app surfaces dynamic `project.session` sessions - spawn-point headers, the session-name dialog, the durable "available" list, the restored/working/Check-terminal chips, terminal-eligibility for any local daemon-drivable session - plus the deferred cross-language surface. Android is locally buildable (`~/android-dev` JDK+SDK; `./gradlew :app:testDebugUnitTest`). Sliced:

- **S1 - cross-language surface (foundational, locally verifiable).** `tests/fixtures/session-id/vectors.json`: add `sessionName` vectors (parse/compose/isComposite). `scripts/codegen-kotlin.ts`: emit `SESSION_SEP` + `DEFAULT_SESSION` into `Protocol.kt` (mirror `GATEWAY_QUALIFIER_SEP`). `android/.../proto/SessionId.kt`: add `parseSessionName`/`composeSessionName`/`isComposite` reading `Protocol.SESSION_SEP`. Tests: `session-id.test.ts` + `SessionIdVectorsTest.kt` iterate the new vectors. Verify: `bun run test` + `./gradlew :app:testDebugUnitTest`.

- **S2 - server: the durable "available" session list + Forget teardown.** `teams()`/`discover()` surface asleep `project.session` entries from the durable resume map as `status:"available"` loose sessions (the resume map doubles as the known-session list), so the app lists sessions that exist but are asleep. A new **kill-session host-op** (`tmuxCore` `kill-session` + the gateway/console wiring); **Forget** drops the resume-map entry AND kills the running tmux (full teardown, Q2). Add a per-session **lastActive** timestamp (updated on send/register), threaded to `TeamInfo` for the recency display + the working/available logic.

- **S3 - the "working" chip (daemon idle/working detection + bounded poll).** Add `isAgentWorking(screen)` (the `esc to interrupt` spinner) beside `isAgentReady`. **Open chat:** an expensive continuous peek poll of that one session sets working/idle live. **Background sessions:** a cheap ONE-SHOT idle check that does NOT rearm (no perpetual loop) - the app requests it when the list is shown. Wire a `working` flag through `TeamInfo`/the app so the chip reflects real agent state, not "armed until reply".

- **S4 - Android board UX.** Devcontainer projects render as collapsible **spawn-point group headers**; `project.session` sessions nest under their project (parse the project segment); host-loose stay a flat group. Tap a header -> **session-name dialog** (validates a slug; existing name opens that child) -> `create_session` with the composite -> open the chat. Tap a child -> open its chat. Chips: **restored/started-fresh** (from the open/wake outcome), **"Check terminal"** (unclean state -> tapping defaults to the Terminal view), **working** (from S3). Expand `terminalEligible` from `kind=='devcontainer'` to any local daemon-drivable session (`project.session` + host). Rename rules updated (a composite session can be relabeled; the project header is the dir name). Bare `project` no longer opens a chat (it is the spawn-point). Verify on-device + `./gradlew :app:testDebugUnitTest`.

**Verification:** TS (`bun run lint`+`test`) and Kotlin (`./gradlew :app:testDebugUnitTest`) green at each slice; the shared vectors pass on both runtimes.

### P4 - audit refinements (refinement lap 1, 6-agent fan-out)

Design verdict: SOUND. Every "blocker" the audit raised is a not-yet-built item it pinned to an exact file - the wiring list, not a design flaw. Refinements:

- **Build order (dependency-correct):** **S1** (cross-language; both runtimes; foundational - S4 needs the Kotlin `parseSessionName`) -> **S2** (server available-list + kill-session/Forget + `lastActive` on `TeamInfo`) -> **S3** (working chip) -> **S4** (board UX; needs S1 Kotlin + S2 list). Each green on its runtime before the next.
- **S1 exact wiring:** `tests/fixtures/session-id/vectors.json` has teamAddress/sessionId/notice/notSession/notNotice but NO `sessionName` block; the TS sessionName tests are INLINE (`session-id.test.ts`), not vector-driven. Add a `sessionName` vectors block (parse/compose/isComposite, incl. bare, dotted-project, composite, trailing-sep edge cases), make `session-id.test.ts` iterate it, add a `@Test sessionNameVectors()` to `SessionIdVectorsTest.kt`. `codegen-kotlin.ts`: import + emit `SESSION_SEP` + `DEFAULT_SESSION` into `Protocol.kt` (mirror `GATEWAY_QUALIFIER_SEP`). `SessionId.kt`: add `parseSessionName`/`composeSessionName`/`isComposite` reading `Protocol.SESSION_SEP` (split on LAST sep).
- **S2 exact wiring:** `RoutesDeps += sessionResume` (pass it from index.ts into `createRoutes`); `teams()` gains a third loop over `sessionResume` emitting asleep composites as `{status:"available", kind:"loose", gatewayId:local, lastActive:lastSeen}` (skip names already online/in-catalog). `TeamInfoSchema += lastActive: z.number().int().optional()`. Kill/Forget: `HostOp += {kind:"killSession",target,dedupKey?}`; `tmuxCore.killSession(target)` = `tmuxArgv(target,["kill-session","-t",sessionName])`; wire it in `hostOpRunner` (mutating, dedup); `ConsoleOpSchema += {kind:"forget", target}`; `isMutatingOp` += forget; a `forget` dispatch case (resolve target, GUARD it is a composite not a bare spawn-point, relayToHost killSession, then drop the resume entry); `ConsoleHandlerDeps += dropSessionResume?(team)` wired to `sessionResume.delete`. App `ChatRepository.forget()` additionally fires the gateway `forget` op. Forget leaves pending jobs to TTL (no extra cleanup).
...
## P5 - detailed design (refinement lap 1)

**Goal:** the host machine is a symmetric spawn point - you spawn `host.<session>` chats the same way you spawn `project.<session>`, owner-gated. The server already does the heavy lifting: `resolveTmuxTarget` maps both a bare `host` and a composite `host.session` to `{kind:"host"}`, `buildLaunchCommand` has the host form (`export PROJECT_NAME=host.<session>; claude ...; exec bash`), `tmuxCore` drives a bare-`tmux` host session, and `killSession`/`ensureSession`/the resume map all treat a `host.session` composite exactly like a devcontainer one (it is a composite loose peer). So P5 is one app slice:

- **S1 (app) - synthetic host spawn-point.** The `host` is NOT in `offlineCatalog` (that holds devcontainer dirs) and the daemon's reserved `host` WS slot is console-hidden, so the board must inject a synthetic `host` spawn-point. Render it ONLY in the LOCAL gateway group (your own machine; you cannot drive a peer's host), as a spawn-point header whose dialog runs `onSpawn("host", session)` -> the existing `createSession("host", session)` -> opens `host.<session>`. `host.<session>` registers as a composite loose, so it nests under the host header through the SAME `renderProject`/`byProject` machinery as devcontainer sessions. Owner-gating = the existing console owner-auth (Q-F3, no extra confirm); restricting the header to the local gateway is the visibility gate.
- **Verify:** Android `:app:testDebugUnitTest` (compile). The host `create_session`/`forget`/working-chip paths reuse the devcontainer code paths already shipped + audited in P4, so no new server surface. On-device spawn of a `host.scratch` is the user's smoke test.

This is deliberately minimal - the P3/P4 work already made the host a first-class composite target; P5 only reveals it in the UI.

**P5 - audit confirmation (2-agent):** SOUND, server path COMPLETE, no design change. Verified: `teams()` filters the reserved daemon `host` slot (`if (name === "host") continue`), so it never collides with the synthetic spawn-point; a `host.session` registers as a normal composite loose (the reserved-name check is exact-match only); `create_session`/`forget`/resume all run the host target through the same code as a devcontainer. Implementation notes from the audit: (1) inject the synthetic host header EVEN when no `host.session` exists yet (so the first one is spawnable); (2) gate it to the LOCAL gateway group (`key.domainId == adminDomainId && key.gatewayId == localGatewayId`), never a peer; (3) `spawn:host` item key is unique. The legacy bare-`host`->`claude` default in `resolveTmuxTarget` is harmless (the app always sends an explicit session name) - a future cleanup, not P5.

## P6 - detailed design (refinement lap 1, final phase)

**Goal:** crosstalk is session-addressable, enabling the 3-way "tell devcontainer X to join THIS chat". Most of the substrate already exists from P3/P4: `teams()` lists composite sessions (online + asleep-from-resume-map with `lastActive`), `/discover` returns the full `TeamInfo[]` (so `lastActive` is already on the wire), and `crosstalk_send` -> `/send` -> `tryWakeTeam` -> `doWakeTeam` already wakes a composite (HIT routes to an online session; a MISS wakes/`--resume`s an asleep one, or launches `project.session` fresh if the project exists). So P6 is focused:

- **S1 (crosstalk_discover) - sessions + recency.** `bridgeDiscover.ts`: read `lastActive` off each discovered `TeamInfo` and render a recency column for asleep sessions ("last seen 5m ago", a small relative-time formatter). HIDE bare spawn-points (`kind === "devcontainer"`) the same way console/host are hidden - an agent addresses a SESSION, not a spawn-point (a MISS creates the session). Update the tool description: the list is sessions; you may target a `project.session` that is not listed (it is created on send). Cross-gateway loose peers still show.
- **S2 (crosstalk_send) - session targeting (mostly description).** `bridgeSend.ts`: the send path already targets a composite via the wake path, so the change is the `to` description - it accepts a `project.session`; a HIT routes to the online session, a MISS wakes/resumes it (or launches it fresh in an existing project). No routing code change (verify the composite `to` flows through `/send` -> `doWakeTeam` unchanged).
- **Verify:** `bun run lint`+`test` (a unit test for the recency formatter + the spawn-point filter). On-device 3-way is the user's smoke test.

**Deferred (P4 crust):** `lastActive` updates on register, not on every send, so asleep recency is "last connected" not "last messaged" - accurate enough for "which session was alive recently". Touching `lastSeen` on send is a small follow-up, not needed for the core 3-way flow (the human's CURRENT chat is online, shown as live, not via recency).

**P6 - audit confirmation (2-agent):** SOUND, no design change. The composite `crosstalk_send` path is verified UNCHANGED - a composite `to` routes when online (HIT) and flows through `/send` -> `doWakeTeam(composite)` to wake/resume/create when offline (MISS); the `doWakeTeam` catalog fail-fast fires only for bare projects (catalog membership), never a composite, so it does not break session sends. All implementation is in `bridgeDiscover.ts`/`bridgeSend.ts` (the agent-facing tools): add `kind !== "devcontainer"` to the filter, the `lastActive` recency formatter, the updated descriptions, and tighten the `kind?` type to `kind`.
...
- `src/gateway/index.ts : startGateway : DATA_DIR migration` - the one-time migration is TODO-tagged for removal (~2026-07). It leaves the legacy `/app/log` copies in place as a backup; once removed, the stale federation-key copies under the log dir should be manually cleaned.
- `src/gateway/console/consoleHandler.ts : resolveTmuxTarget : bare->claude` - the terminal view still maps a bare `project` to the `claude` session for legacy addressing, but in the clean break a bare project has no live `claude` session, so a bare-project peek reads "no server running". Harmless (P4's "Check terminal" chip covers it), but the bare->claude default could be dropped once the app only ever sends composites.
- `src/mcp/devcontainer/hostDaemon.ts : handleWake : Hypothesis J/K/N debug blocks` - P3 rewrote handleWake but PRESERVED the `#region Hypothesis` debug-log scaffolding (per the no-sweep rule). It is still shipped; folds into the debug-log cleanup painpoint.
