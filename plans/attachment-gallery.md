...
### 9. Video previewables - the animated THUMB only - ✅ DONE

**Verified on the emulator against a real encoded clip**, generated by the sandbox rather than
bundled, with a deliberately sparse 10s GOP because that is what makes a keyframe-snapping seek
collapse. A 40s clip produced exactly 6 frames, which is what the rule predicts (8 points, ends
dropped), and all 6 were byte-distinct. That single observation disproves BOTH of the failure modes
named below at once: a default seek would have collapsed them onto the sync frames, and a ms-for-us
slip would have parked every sample near zero. The tile was sampled cycling through green, blue,
purple and dark red.

The frame set landed AFTER its row had rendered and the row picked it up, which is the readiness
push channel working with both halves attached.

**`MIN_POINTS` was raised rather than renamed.** At 3 the rule kept `n-2 = 1` frame, so the constant
promised a floor on motion while yielding a static thumb, exactly as the plan warned. It is
`MIN_MOTION_POINTS = 4` now, the first n that leaves two frames, and a test asserts that any
non-empty result genuinely moves.

**Same false premise as section 7, and the same resolution.** Frame sets are NOT shared "by
construction" through `blobId`: a draft video has none until send. `VideoThumbs.keyFor` is
`blobId ?: src`.

Video is still absent from `PREVIEWABLE_MIMES`. That set answers for surfaces holding only the file,
