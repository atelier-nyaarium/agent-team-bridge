...
- [medium] **`clearDone` and `sessionEnded` can trash a parent out from under a surviving child.**
  ...
  at all. The result is a parent pointer no list resolves, which self-heals only at the 30-day sweep.
  `liveParents` should be "has any child this pass will not trash".
