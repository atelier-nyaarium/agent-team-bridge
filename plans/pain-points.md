...
- [low] `android/.../Sharing.kt : SharingScreen` - **bug-class** - the per-session share-audience map (`shares`, Private/Everyone/Specific) is populated only by an explicit `refresh()` call (on mount, and after a local mutation), never when `sessions`/`state` changes for an external reason - a session that newly appears in the list while the sheet is open has no `shares` entry yet and silently renders as "Private" until the next manual refresh. Pre-existing, unrelated to the id-teardown work; would need `refresh()` wired to the same state-driven recomposition the id-teardown pass added for `sessions`/`people`.
