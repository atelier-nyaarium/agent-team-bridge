...
## P3 Arbiter relay wiring + peer hardening

STATUS (lap 1, post align-fix x2, 126/126 tests, tsc 0, lint 0): implemented + two audit rounds applied. Beyond the original spec, the align audits added: mailbox accessor + onEvict->removePeer (peer lifetime = mailbox lifetime; TTL sweep cannot orphan deliveries); virtual peers excluded from DM holder selection (getAllActiveRealWs in pickFirstOnlineTeam/tryDeliverDm) and from getTeamMode; websocket register evicts virtual squatters (+ onVirtualPeerEvicted -> removePeer clears binding/mailbox) and close() cleanup ignores virtual subs; CLI-target sends rejected when online and recovered via a post-bound continuation when woken (wake failures and CLI answers land in the mailbox as reply entries; sendBoundMs injectable, default 25s); register op may rename a device (rebind migrates the registry sub, mailbox preserved; non-register ops still locked); conversationRegistry pinning extended vs real sockets + pointer self-heal; zod PhoneRelayFrameSchema validated in an extracted createPhoneRelayPump (tested); routes.respond team-broadcast fallback gated to entries WITHOUT fromConversationId (conversation-routed sends never degrade to name-broadcast - matches the code comment's documented intent); missing crypto import in websocket.ts fixed; biome excludes plans/. Known accepted-minor: pickFirstOnlineTeam/tryDeliverDm remain non-exported closures (virtual-filter behavior reviewed, not unit-tested).
