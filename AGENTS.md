...
- `src/gateway/vault/decisions.ts` / `requests.ts` / `helperTokens.ts` / `vaultRoutes.ts` - grants and the display shape; request rows that settle once, admit a joining caller, and cap open requests per target; helper tokens; loopback routes
