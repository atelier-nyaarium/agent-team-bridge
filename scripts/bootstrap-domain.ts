...
// tenant - an operatorName label plus a one-time invite nonce, no owner root - and the operator's
...
// path, prior admissions / revocations + the operatorName are kept ONLY when re-rooting at the SAME
...
/** A pending-tenant record on a not-yet-rooted Domain (an operatorName display label + a
 ...
 * (no owner has rooted it yet) and set once rooted. `operatorName` is the friendly network
 ...
 * carrying every field (including a friend Domain's operatorName + pendingTenant) through
...
function carryOtherDomain(slice: Partial<DomainEnrollment> | undefined): DomainEnrollment {
	return {
		...
		...(slice?.operatorName !== undefined ? { operatorName: slice.operatorName } : {}),
 ...
 * operatorName only when re-rooting at the SAME owner; a different owner key is a fresh Domain
 * (prior admissions would not verify under it, and its operatorName was the prior owner's label,
 * so both are dropped).
 *
 ...
 * live re-provision preserves operatorName by NOT TOUCHING the slice, not through this helper. This
 ...
 * [], revocations: [], operatorName, pendingTenant }`) so evie reads it back without complaint. The
 * nonce is minted by the caller (standard base64, never base64url - the wire `nonce` is a b64Field). */
function pendingHomeSlice(operatorName: string, nonce: string, issuedAt: number, ttlMs: number): DomainEnrollment {
	return {
		ownerSignPub: null,
		ownerBoxPub: null,
		admissions: [],
