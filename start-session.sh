...
# Launches a Claude conversational session in a detached tmux session named <name> on this machine.
# It joins the gateway as an ordinary loose peer (the privileged host-agent was retired in the host
# split - the host plumbing now lives in the headless host daemon, see start-host-daemon.sh). <name>
# is the tmux session name: attach with `tmux attach -t <name>`, and the console terminal view drives
# it by that name. Defaults to "claude".
...
tmux new-session -d -s "$NAME" "bash -c 'source ~/.bashrc; claude --name ${NAME} --model default --effort low --dangerously-skip-permissions --dangerously-load-development-channels plugin:switchboard@atelier-nyaarium; exec bash'"

# Wait for Claude to start, auto-accept dev channels prompt if it appears
for i in $(seq 1 10); do
	sleep 1
	SCREEN=$(tmux capture-pane -t "$NAME" -p 2>/dev/null || true)
