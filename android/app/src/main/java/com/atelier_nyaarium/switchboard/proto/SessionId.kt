...
class TeamAddress private constructor(val host: String, val name: String) {
	companion object {
		/** A local team. A bare name resolves to localHostId; an already-qualified
		 * name keeps its (possibly remote) host. Idempotent. */
		fun local(localHostId: String, name: String): TeamAddress = parse(name, localHostId)

		/** An explicit, possibly-remote host; NOT re-resolved to local. */
		fun remote(host: String, name: String): TeamAddress = TeamAddress(host, name)

		/** Parse a wire team string. The FIRST separator splits host from name; a
		 * bare name resolves to localHostId. An explicit host is preserved. */
		fun parse(team: String, localHostId: String): TeamAddress {
			val i = team.indexOf(Protocol.HOST_QUALIFIER_SEP)
			return if (i == -1) {
				TeamAddress(localHostId, team)
			} else {
				TeamAddress(team.substring(0, i), team.substring(i + Protocol.HOST_QUALIFIER_SEP.length))
			}
		}
	}

	/** The one canonical string form: host + SEP + name. */
	val canonical: String get() = "$host${Protocol.HOST_QUALIFIER_SEP}$name"

	override fun equals(other: Any?): Boolean = other is TeamAddress && host == other.host && name == other.name
...
class SessionId private constructor(val conversationId: String, val target: TeamAddress) {
	companion object {
		...
		fun parse(wire: String, localHostId: String): SessionId? {
			...
			return SessionId(conversationId, TeamAddress.parse(team, localHostId))
		}
	}
...
class NoticeId private constructor(val sender: TeamAddress) {
	companion object {
		...
		fun parse(wire: String, localHostId: String): NoticeId? {
			...
			return NoticeId(TeamAddress.parse(sender, localHostId))
		}
	}
