		...
		return domains
			.map { domainId ->
				...
				LinkedDomain(
					...
					// each shared session's profileName). First non-empty wins; null until any session
					// carries one, in which case the UI falls back to the opaque domainId.
					profileName = sessions.firstNotNullOfOrNull { it.profileName?.ifEmpty { null } },
			...
			.sortedBy { it.profileName ?: it.domainId }
	}
}
