...
- **Recovery** (clean-break, owner has 1 console + 1 gateway): run `provision-admin-domain.sh` `0) Purge Federation`. It drops only this owner's Domain slice from evie's Secret (a hosted friend tenant survives), then does the same full local wipe as Purge gateway (`.env` + `volumes/gateway`) and removes the host blob under `~/.config/switchboard`, then re-run setup. There is no silent re-root: a snapshot / redeem for a different owner is refused.
