...
- `start-gateway.sh` - Plain start for this machine's gateway (git pull, rebuild, health wait); setup lives in `./provision-admin-domain.sh --setup`
- `provision-admin-domain.sh` - Thin launcher: execs `scripts/provision.ts` (the admin home Domain bootstrap; roots the owner's own Domain, not a path to add consoles or guests)
...
- `scripts/provision.ts` - The unified admin setup flow (the `provision-admin-domain.sh` logic, in bun): a menu that configures the gateway (GATEWAY_ID + owner key, mints the Domain id, rebuild + start), creds-less LAN-enrolls a secondary gateway into an existing Domain, provisions the home Domain (cluster cutover, Domain rooting at the owner key, transport-blob emit + enrollment QR, authenticated bridge verify), and purges the gateway or the federation
...
- **Recovery** (clean-break, owner has 1 console + 1 gateway): delete the `evie-federation` Secret, each gateway's `federation-allowlist.json` (keep `identity.json`), and `~/android-dev/secrets/console-owner-identity.json`, then re-run `./provision-admin-domain.sh --setup`. There is no silent re-root: a snapshot / redeem for a different owner is refused.
...
Recovery (clean-break): delete the `evie-federation` Secret + each gateway's `<dataDir>/federation/federation-allowlist.json` (keep `identity.json`) + `~/android-dev/secrets/console-owner-identity.json`, then re-run `./provision-admin-domain.sh --setup`. A redeem / snapshot for a different owner is refused by design, so there is no silent takeover.
