...
- `scripts/provision.ts` - Admin home Domain bootstrap flow (the `provision-admin-domain.sh` logic, in bun): Provision/Purge menu, cluster cutover, Domain rooting at the owner key, transport-blob emit + enrollment QR, and an authenticated bridge verify
