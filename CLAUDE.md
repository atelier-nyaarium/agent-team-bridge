...
- `provision-admin-domain.sh` - Thin launcher: execs `scripts/provision.ts` (the admin Domain bootstrap; roots the owner's own Domain, not a path to add consoles or guests)
...
- `scripts/provision.ts` - The unified admin setup flow (the `provision-admin-domain.sh` logic, in bun): a menu that configures the gateway (GATEWAY_ID + owner key, mints the Domain id, rebuild + start), creds-less LAN-enrolls a secondary gateway into an existing Domain, provisions the admin Domain (cluster cutover, Domain rooting at the owner key, transport-blob emit + enrollment QR, authenticated bridge verify), and purges the gateway or the federation
...
- **Recovery** (clean-break, owner has 1 console + 1 gateway): delete the `evie-federation` Secret, each gateway's `federation-allowlist.json` (keep `identity.json`), and `~/android-dev/secrets/console-owner-identity.json`, then re-run `./provision-admin-domain.sh`. There is no silent re-root: a snapshot / redeem for a different owner is refused.
...
2. **Required config**: `FEDERATION_DOMAIN_ID` has no default; the gateway fails closed at boot if it is unset. The admin's Domain id is minted and written to the gateway `.env` by `provision-admin-domain.sh`. A creds-less secondary gateway (the Enroll gateway option under `provision-admin-domain.sh`) does not yet receive it (the sealed bootstrap bundle carries no Domain id), so until that is plumbed through it must be set in `.env` by hand first.
3. **Onboard the Console with `./provision-admin-domain.sh` plus the app's onboarding scan**. There is NO owner-key paste: the owner root key is generated silently on the phone, so `provision.ts` is a fresh-vs-reprovision state machine. A FRESH setup STAGES A PENDING admin Domain (no host-side rooting), PROMPTS for the display name (you type "Nyaarium"), applies the console-bridge + gateway-bridge cutover (above), writes the Gateway's `bootstrap-transport.json`, restarts evie, then emits a TRANSPORT-ONLY provisioning blob to `~/android-dev/secrets/console-provisioning.json` (0600; the CONSOLE-bridge SA creds, NOT the gateway-bridge transport) carrying a `pendingTenant{domainId, nonce}`. On scan the phone reads `pendingTenant` off the blob, self-signs a `FIRST_ROOT_V1` with its silent owner key, and first-roots the Domain EVIE-DIRECT (see Friend onboarding above); the gateway self-heals once the phone roots the admin Domain. The Console then generates its OWN member identity, owner-signs a `kind:console` admission, and submits it to evie (it likewise owner-admits each Gateway it enrolls). The Gateway id is read authoritatively from the container's `GATEWAY_ID` env (run through `sanitizeGatewayId`), never from `docker logs`. Credential posture: the blob is a durable, host-local 0600 file carrying the cluster SA token only (NO identity, no private keys). RE-PROVISIONING an already-rooted admin Domain skips bootstrap and re-emits the blob only (no re-root). For the untrusted-evie model, set `FEDERATION_OWNER_SIGN_PUB` to the owner key on each gateway; the script ABORTS with the exact remediation if the gateway pins a different owner than the one just rooted.
4. **Pin the owner (untrusted-evie model)**: set `FEDERATION_OWNER_SIGN_PUB` to the owner key (and `FEDERATION_REQUIRE_OWNER_PIN=true`) on every gateway, so a malicious / SA-token-holding evie cannot trust-on-first-use a fresh Gateway onto an attacker-rooted Domain. evie gates every `gateway_register` on the owner-signed admission, so a Gateway with no admission is rejected; a fresh Gateway must be enrolled (admit-gateway QR or the creds-less `--enroll` bundle) before it can register.

Recovery (clean-break): delete the `evie-federation` Secret + each gateway's `<dataDir>/federation/federation-allowlist.json` (keep `identity.json`) + `~/android-dev/secrets/console-owner-identity.json`, then re-run `./provision-admin-domain.sh`. A redeem / snapshot for a different owner is refused by design, so there is no silent takeover.
