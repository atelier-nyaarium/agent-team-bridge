...
- **Enrollment** (evie's `EnrollmentCoordinator`, persisted in the `evie-federation` k8s Secret via the in-cluster SA + REST - `KubeSecretStore`): three typed-QR flows, each anti-MITM via a SAS fingerprint the human confirms out-of-band. (1) **enroll-owner**: the owner asks evie (the owner-only `federation-enroll-owner` action), evie mints a single-use nonce QR and DMs it; the console scans, generates its owner identity, and POSTs `enroll_redeem` to root the Domain. (2) **admit-switch**: a not-yet-admitted arbiter prints its admit-switch QR + SAS on startup (`enrollQr.ts`, ANSI-forced contrast); the owner scans, owner-signs the admission, and POSTs `submit_admission`; evie records it and mirrors it back. (3) **authorize-console**: a second owner device (display side pending). The console's enroll ops ride the same app-token bridge but are handled DIRECTLY at evie (the Domain root), never relayed to a Switch, and are self-authenticating (the mint nonce or an owner signature). The Android scanner + flows live in `android/.../enroll/`.
...
- **Recovery** (clean-break, owner has 1 console + 1 arbiter): delete the `evie-federation` Secret and each arbiter's `federation-allowlist.json` (keep `identity.json` so the arbiter re-presents its admit-switch QR), then re-enroll. There is no silent re-root: a snapshot / redeem for a different owner is refused.
