...
- `src/` - Main source code
  ...
  - `gateway/` - **Gateway server** - A Gateway: the central HTTP + WebSocket router for one machine's teams, running in Docker
    ...
    - `console/` - **Console bridge** - gateway side of the Android channel (see Console Bridge below)
      - `consoleHandler.ts` - `createConsoleHandler`: validates device identity, dispatches the console ops (register/list_teams/send/respond/poll plus the terminal-view ops) by reusing the HTTP routes, owns per-conversation mailbox/binding/idempotency state
...
- `start-gateway.sh` - Plain start for this machine's gateway (git pull, rebuild, health wait); setup lives in `./provision-admin-domain.sh`
- `provision-admin-domain.sh` - Thin launcher: execs `scripts/provision.ts` (the admin setup menu; sets up this machine's gateway and roots the owner's own Domain, not a path to add consoles or guests)
...
- `tests/fixtures/_signing-vectors-manifest.json` - the cross-runtime inventory of the signing-bytes vector corpora (provision-ops, xdomain-link, cross-domain-sas, enroll-sas, session-id, sync-cursor, transport-request); a vitest test + a Kotlin twin iterate it and assert directory agreement, so a new op's vectors can never be read by only one runtime (the stale-vector-breaks-on-main class). Per-op suites still own the byte assertions
...
- **Recovery** (clean-break, owner has 1 console + 1 gateway): run `provision-admin-domain.sh` `0) Purge Federation`. It drops only this owner's Domain slice from evie's Secret (a hosted friend tenant survives), wipes this gateway's `federation-allowlist.json` (keeping `identity.json`), and removes the host blob + legacy `console-owner-identity.json` under `~/android-dev/secrets`, then re-run setup. There is no silent re-root: a snapshot / redeem for a different owner is refused.
...
- **Ops:** `register`, `list_teams`, `send` (to a devcontainer or a loose session), `respond` (only to a thread delivered to this device), `poll`, `peek` + `tmux_send` + `create_session` + `reload_plugins` (the terminal view + session lifecycle: see Console terminal view below). `send`/`respond`/`tmux_send`/`create_session`/`reload_plugins` are idempotent per `(conversationId, opId)`; reads (`poll`/`peek`) run fresh. The old gateway-served `get_gateway_transport` op is RETIRED: the value stays in the wire union for codegen continuity but the Gateway throws on it (an old build gets a clear error). The Console now pulls the gateway-bridge transport from evie (see Gateway-bridge transport pull above).
...
- `FEDERATION_DIR` - Where this Gateway persists its keypair + mirrored allowlist + the enrollment-delivered transport.json + domain-id (default: alongside the log path)
...
Per-user purges: `9) Purge Gateway` drops only this gateway's admission from evie's Domain (`removeGatewayAdmission`) then erases the local `.env` + `volumes/gateway`. `0) Purge Federation` drops only the running user's whole Domain slice from evie (`removeDomain` - other tenants, including their cross-Domain link edges, survive), then wipes this gateway's mirrored allowlist (keypair kept) and the host blob under `~/android-dev/secrets`. Re-run `./provision-admin-domain.sh` afterward. A redeem / snapshot for a different owner is refused by design, so there is no silent takeover.
