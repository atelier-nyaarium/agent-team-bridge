...
- `src/` - Main source code
  ...
  - `arbiter/` - **Arbiter server** - A Switch: the central HTTP + WebSocket router for one machine's teams, running in Docker
    ...
    - `console/` - **Console bridge** - arbiter side of the Android channel (see Console Bridge below)
      - `consoleHandler.ts` - `createConsoleHandler`: validates device identity, dispatches the console ops (register/list_teams/send/respond/poll) by reusing the HTTP routes, owns per-conversation mailbox/binding/idempotency state
    ...
    - `federation/` - **Federation** - cross-Switch routing + trust (see Federation routing + Federation trust below)
      - `hostRelay.ts` - `createSwitchRelayHandler` (runs an inbound federated op - send/list_teams/wake/response_push - against the local routes) + `createSwitchRelayPump` (zod-validates each `switch_relay` frame, dispatches, replies as a `switch_relay_reply` tool call). The cross-Switch mirror of the console relay pump. Opens/seals the payload through the `sealer`
      - `identity.ts` - `loadOrCreateIdentity(dataDir)`: mints + persists this Switch's keypair (0600) on first boot
      - `allowlist.ts` - `Allowlist`: the mirrored Domain (owner root + owner-signed admissions/revocations) on the Switch's volume; `applySnapshot` (idempotent mirror of evie's push), `resolveSwitch`/`resolveBySignPub`, `selfAdmission`
      - `sealer.ts` - `createSealer(identity, allowlist, replayGuard?)`: `seal(dstSwitch, obj)` / `open(srcSwitch, env)` - E2E peer-to-peer over the allowlist's keys, replay-checked after the signature verifies
      - `replayGuard.ts` - `ReplayGuard`: TTL + capped seen-set rejecting a replayed authentic sealed frame
      - `enrollQr.ts` - `logAdmitSwitchQr`: prints this Switch's admit-switch QR + SAS (ANSI-forced contrast) on startup while un-admitted
  ...
  - `mcp/` - **MCP plugin** - Tools registered for Claude Code and other IDE agents
    ...
    - `channel/` - **Channel mode** - For Claude agents receiving push notifications
      ...
      - `humanTools.ts` - the `notify_human` tool: broadcasts a `{title, summary, full, attachments?}` notice (all three tiers required; `tiny` is a deprecated alias for `title`) to every registered console via the arbiter's `POST /human/notify`. `attachments` are absolute file paths the agent attaches from its filesystem
  ...
  - `shared/` - Shared utilities used by both arbiter and MCP
    ...
    - `session-id.ts` - The Session Identity value objects (the one canonical address/session form): `TeamAddress` (a `switch/name` address; a bare name resolves to the local Switch at the boundary, an explicit/remote switch is preserved so cross-Switch keys stay byte-stable across arbiters), `SessionId` (a channel conversation's `conv:<conv>:<switch/name>` key - the single canonical producer, with an injective parse that rejects crafted multi-colon ids), and `NoticeId`. A store key and a lookup key are the SAME value by construction, replacing the ad-hoc bare-vs-qualified string builders. Owns the grammar constants (`CONV_SESSION_PREFIX`/`NOTICE_SESSION_PREFIX`/`SWITCH_QUALIFIER_SEP`). Hand-authored Kotlin twin at `android/.../proto/SessionId.kt`, held equivalent by `tests/fixtures/session-id/vectors.json` read by both runtimes
    - `host-id.ts` - The arbiter's own Switch id: `resolveLocalSwitchId` (the `SWITCH_ID` env override, else the sanitized machine hostname) and `sanitizeSwitchId` (slug to `[a-z0-9-]`, never the qualifier separator). Threaded through `ArbiterConfig.localSwitchId`
    - `evie-protocol.ts` - SELF-CONTAINED (zod-only) leaf owning the arbiter<->evie frame vocabulary, SYNCED verbatim into evie-bot (`cp` + SYNC-HASH): `EvieInboundFrameSchema` (tool_registry / tool_result / tool_error / loose console_relay + loose switch_relay), `ToolCallFrameSchema`, `ChannelFileSchema` (re-exported by schemas.ts), and the federation routing envelope evie switches on - `SwitchRegisterParamsSchema`, `SwitchRelayRouteSchema` (opaque `payload`), `SwitchRelayReplyParamsSchema`, `FEDERATION_PROTOCOL_VERSION`. The register params also carry the optional admitted-identity proof (signPub/boxPub/admission-JSON/proof). Nothing imports into it
    ...
    - `enrollment.ts` - SYNCED leaf (imports `./admission.js` + `./crypto.js`): the typed QR payloads (`EnrollmentPayloadSchema`: enroll-owner/admit-switch/authorize-console), the owner enroll ops (`EnrollOpSchema`: enroll_redeem/submit_admission/submit_revocation, codegen'd sealed), `EnrollResult`, `payloadSas`, `admissionFromScan`
    - `federation-protocol.ts` - switchboard-only INNER federation vocabulary evie never sees (content-blind): `FederatedOpSchema` (send/list_teams/wake/response_push), the `ReturnRouteSchema` reply-pin, the `SwitchRelayPayloadSchema` (always E2E-sealed: a single `sealed` envelope, no cleartext op - evie cannot read or forge it), and the full `SwitchRelayFrameSchema` the switch-relay pump validates. NOT codegen'd to Kotlin (cross-Switch is arbiter-to-arbiter; the console reaches the mesh through its home Switch)
...
The arbiter maintains a kubectl port-forward tunnel to evie-bot's K8s pod (port 20001). A WebSocket client (`evieClient.ts`) connects with bearer auth. Its live role is the console relay transport: evie relays the console's `console_relay` frames over this WS, and the arbiter answers each with a `console_relay_reply` tool call (see Console Bridge below). The same tool-call wire (`callTool` + `tool_result` / `tool_error` correlation) carries those replies.
...
**Outbound (agent -> console):** `notify_human` accepts `attachments` as absolute file paths; the MCP plugin reads each with `fs.readFile`, base64-encodes it into a `ChannelFile`, and ships it on the `/human/notify` notice. The arbiter appends the notice (with files) to every registered console's mailbox.

### Switch identity and qualified names

Every arbiter is a **Switch** with an id (`SWITCH_ID`, else the sanitized machine hostname; see `resolveLocalSwitchId`). Session names are **switch-qualified** as `switch/name` on the wire and on the console; a BARE name (no separator) resolves to the local Switch. The arbiter stays keyed by bare local names internally and qualifies only at the wire edge: `teams()` stamps each `TeamInfo.switchId`, register returns the connected `switchId`, and `send` canonicalizes an inbound target (stripping the local Switch, 404ing a different one) so the channel session id carries `switch/name`. The console keys every per-session surface (threads, tabs, unread, labels) by the canonical `switch/name` value through its `SessionId.kt` twin, learns its Switch id at register, and load-normalizes its persisted threads/labels to canonical on read; its presence list compares canonical values, so a session that is live can never be synthesized as a phantom "ended". The arbiter derives every session/team key through the `SessionId`/`TeamAddress` value objects (`shared/session-id.ts`, the one canonical producer, so a store key and a lookup key are the same value by construction); the separator/prefixes are codegen'd into Kotlin and the console mirrors the grammar in its `SessionId.kt` twin. Cross-Switch routing builds on this (see Federation routing below).

### Federation routing (multi-Switch mesh)

evie is the content-blind **Router** over many Switches (arbiter side in `arbiter/federation/`; evie side is evie-bot's `BridgeServer`). Each arbiter REGISTERS its switch id on connect (`switch_register`), keying a switch id -> socket table at evie. A Switch reaches another by calling evie's `switch_relay` tool; evie switches the frame to the destination Switch's socket by `dstSwitch` alone and correlates the reply by `relayId` (held open, bound to the destination's connection), never parsing the payload. A cross-Switch send (`routes.sendCrossSwitch`) forwards a `switch_relay` carrying a **return-route** `{srcSwitch, srcConversationId, srcSession}`; the destination lands a local channel_push keyed by that same session id with the return-route on its job, and when its agent replies, `respond` forwards a `response_push` home through evie to the origin's anchor (which pushes into the originating conversation). Discovery (`routes.discover`, behind `crosstalk_discover` + the console's `list_teams`) fans out a `list_teams` over evie's presence roster (`list_switches`) and merges - evie aggregates no team lists. The console reaches the mesh through its **home Switch** (the id it learned at register, sent as `homeSwitch` on every relay; evie routes there, else falls back to the latest arbiter). Cross-Switch frames are now SEALED end to end (see Federation trust below): `switch_relay.payload` carries a `SealedEnvelope` only the destination Switch can open. On a Switch drop, evie's `onDisconnect` fails that Switch's in-flight relays with an explicit error (not a stall); the arbiter's evie-client auto-reconnects and re-syncs the allowlist on re-register.

### Federation trust, enrollment, and E2E crypto

The mesh runs on a single root of trust: the **owner** device. Membership is an allowlist of owner-signed **admissions** mirrored on evie AND every Switch, so a revocation bites even while evie is unreachable (audit R3).
...
- **Trust model** (`shared/admission.ts`, SYNCED): `AdmissionSchema` (owner attests a subject's keys, `kind: switch|console`), `SignedAdmission`, `Revocation`. Canonical signing bytes are a versioned newline-joined encoding (`ADMISSION_V1\n...`) reproduced byte-exact on node/Bun/Android - never sign raw JSON. `resolveAdmitted` returns the newest owner-verified, non-revoked admission. `DomainSnapshot` = the mirrored `{ownerSignPub, admissions, revocations}`.
- **Registration gate**: `switch_register` carries the Switch's `signPub`/`boxPub`, its owner-signed `admission` (JSON), and a fresh `proof` (Ed25519 over `REGISTER_V1\nswitchId\nproofAt`, proving key possession so an observed admission cannot be replayed). evie's `verifyRegistration` rejects an unadmitted / revoked / replayed / stale registration. During the token+admission coexistence window a token-only register still passes (so a not-yet-enrolled arbiter can connect and pull its first sync); `FEDERATION_REQUIRE_ADMISSION=true` makes the proof mandatory at token retirement.
- **Domain mirror**: evie returns the `DomainSnapshot` in the register reply; the arbiter's `Allowlist.applySnapshot` idempotently replaces its state with the owner-verified entries (dropping any forged one), so each Switch enforces the allowlist + revocations locally.
- **Enrollment**: the OPERATOR path is now the host-side `./provision-console.sh --setup` (see "Deploying the federation" below). It roots the Domain + admits the Switch + a Console DIRECTLY on the trusted host (the owner root key never leaves the host) and emits a 0600 blob the app imports - no QR, no SAS, no evie-DM. The in-app scanner under `android/.../enroll/` and the `federation-enroll-owner` DM action are RETIRED. The underlying trust machinery is unchanged and still lives in evie's `EnrollmentCoordinator` (persisted in the `evie-federation` k8s Secret via the in-cluster SA + REST - `KubeSecretStore`): owner-signed admissions, the `EnrollOp` ops, and the typed-QR payloads remain for a possible future couch-side authorize-console (a second owner device), but are not part of first-run onboarding. The crypto in `scripts/bootstrap-domain.ts` reuses the same `crypto.ts`/`admission.ts` so a host-minted admission verifies byte-for-byte on the arbiter and the app.
- **Replay-reject**: `ReplayGuard` (a TTL + capped seen-set, checked AFTER the signature verifies) rejects a captured authentic cross-Switch frame re-delivered to re-run the op.
- **Recovery** (clean-break, owner has 1 console + 1 arbiter): delete the `evie-federation` Secret, each arbiter's `federation-allowlist.json` (keep `identity.json`), and `~/android-dev/secrets/console-owner-identity.json`, then re-run `./provision-console.sh --setup`. There is no silent re-root: a snapshot / redeem for a different owner is refused.

The console<->arbiter op path is now E2E sealed too (the symmetric twin of the cross-Switch seal): the console seals each op into a `ConsoleRelayFrame` (`{ opId, signerSignPub, sealed }`) to the arbiter's box key and signs with its enrolled key; the arbiter verifies the signer against an owner-signed `kind:console` admission, decrypts, replay- and freshness-checks, and seals the reply back. evie stays content-blind (its `console_relay` member is a loose passthrough). Only a pre-seal failure (malformed / unadmitted) returns a cleartext error so the console can prompt enrollment. Still pending on the crypto track: the evie self-provisioning of the console-bridge k8s objects (manual YAML for now), and the operational token retirement (`FEDERATION_REQUIRE_ADMISSION=true` + dropping `BRIDGE_TOKEN`/`CONSOLE_BRIDGE_TOKEN`).

### Console Bridge (Android channel)

Arbiter side of a native Android chat client that reaches the bridge through evie (the only host the console and the home-NAT arbiter both reach). The console is a poll-based client, not a live socket: evie relays opaque `console_relay` frames over the existing arbiter<->evie WS, and the arbiter answers each with a `console_relay_reply` tool call. See `arbiter/console/`; the full design doc lives in git history (`plans/done/android-channel-app.md` at commit f08e83d, removed from the tree after shipping).
...
- **Host-agent:** the host orchestrator's `arbiter` channel identity is surfaced to the console as `kind: "switch"` (shown first), reachable by `send` from the console only (`channelOnly`). A send injects a `channel_push` into the orchestrator, which can dispatch to its devcontainers. The cli `host` wake-daemon stays hidden; container crosstalk to the host-agent is deferred to the federation phases.
...
- `SWITCH_ID` - This Switch's id, qualifying every local session name on the wire (default: the sanitized machine hostname)
...
- `FEDERATION_OWNER_SIGN_PUB` - The Domain owner's raw Ed25519 signing public key (base64), PINNED out-of-band. When set, the Switch refuses any allowlist snapshot rooted at a different key, so a malicious/token-holding evie cannot root a fresh Switch at an attacker key. Unset = trust-on-first-use (convenient; pin it for the untrusted-evie threat model). The owner reads this key from the app after enrolling.
- `FEDERATION_DIR` - Where this Switch persists its keypair + mirrored allowlist (default: alongside the log path)
...
- **Fetch the console's live log** (the only path into the firewalled K8s is evie's stdout, which the arbiter's kubectl already reads):
...
The console bridge spans two repos and three runtimes (evie pod, k8s objects, host arbiter), so a deploy is a fixed sequence. evie deploys via GitHub CI (push to main builds the image and rolls out k8s); the arbiter is a local Docker container rebuilt on the host.
...
4. **Restart the arbiter + host daemon** on the host: `./down.sh && ./start-arbiter.sh && ./start-host-daemon.sh`. `start-arbiter.sh` pulls main and rebuilds the container, so the new arbiter has the `console_relay` handler and reconnects to evie's bridge.
...
Order matters: the console bridge can serve `health` as soon as the env is set, but `register`/`send`/`list_teams` relay through the arbiter, so they only succeed once the arbiter is rebuilt (step 4). Setting the env before applying `console-bridge.yaml` enables the bridge inside the pod but leaves it unreachable until the Service exists.
...
3. **Onboard the Console with the one host-side script**: `./provision-console.sh --setup` (mirrors `start-arbiter.sh --setup`). It is now the SOLE bootstrap authority - there is no in-app QR/scan and no evie-DM (the in-app enroll UI + the `federation-enroll-owner` DM action are retired). It: applies the console-bridge cutover (above), mints the Domain OWNER root key HOST-SIDE (persisted 0600 at `~/android-dev/secrets/console-owner-identity.json`, reused on later runs), owner-signs a `kind:switch` admission for this arbiter + a `kind:console` admission for a fresh Console identity (MERGED into evie's existing enrollment, not clobbering other Switches/revocations), writes evie's `evie-federation` Secret, restarts evie, then emits the provisioning blob to `~/android-dev/secrets/console-provisioning.json` (0600) and verifies the bridge with an authenticated probe. The Console imports that blob (paste/file) and is enrolled from it ALONE. The Switch id is read authoritatively from the container's `SWITCH_ID` env (run through `sanitizeSwitchId`), never from `docker logs`. Credential posture (owner-accepted deviation from the original "short-lived bootstrap token"): the blob is a durable, host-local 0600 file carrying the cluster SA token + the Console PRIVATE key. Re-running `--setup` is the rotation/recovery path; it de-admits the prior Console by snapshot replacement. For the untrusted-evie model, set `FEDERATION_OWNER_SIGN_PUB` to the owner key on each arbiter; the script ABORTS with the exact remediation if the arbiter pins a different owner than the root it just minted.
4. **Token retirement** (final, AFTER a full round trip validates): pin `FEDERATION_OWNER_SIGN_PUB` (and set `FEDERATION_REQUIRE_OWNER_PIN=true`) on every arbiter FIRST, so once the bearer is gone a malicious / token-holding evie cannot trust-on-first-use a fresh Switch onto an attacker-rooted Domain. Then set `FEDERATION_REQUIRE_ADMISSION=true` so evie rejects a token-only register, and drop `BRIDGE_TOKEN` once every Switch presents an admission. Setting `FEDERATION_REQUIRE_ADMISSION` rolls the evie deployment, so every Switch reconnects and re-registers under the gate; a Switch that only ever registered token-only is dropped at that reconnect.

Recovery (clean-break): delete the `evie-federation` Secret + each arbiter's `<dataDir>/federation/federation-allowlist.json` (keep `identity.json`) + `~/android-dev/secrets/console-owner-identity.json`, then re-run `./provision-console.sh --setup`. A redeem / snapshot for a different owner is refused by design, so there is no silent takeover.
