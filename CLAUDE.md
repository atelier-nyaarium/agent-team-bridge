...
4. **Token retirement** (final, AFTER a full round trip validates): set `FEDERATION_REQUIRE_ADMISSION=true` so evie rejects a token-only register, then drop `BRIDGE_TOKEN` once every Switch presents an admission.
