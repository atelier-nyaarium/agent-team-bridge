...
**Gateway** (`main-gateway.ts`) - Runs in a Docker container. A Gateway (one of many in the Mesh): the central HTTP + WebSocket router that all local teams connect to. Handles message routing, request/response lifecycle, and the evie-bot bridge (evie is the content-blind Router that forwards between Gatewayes).
...
3. **Onboard the Console with the one host-side script**: `./provision-console.sh --setup` (mirrors `start-gateway.sh --setup`). It is now the SOLE bootstrap authority - there is no in-app QR/scan and no evie-DM (the in-app enroll UI + the `federation-enroll-owner` DM action are retired). It: applies the console-bridge cutover (above), mints the Domain OWNER root key HOST-SIDE (persisted 0600 at `~/android-dev/secrets/console-owner-identity.json`, reused on later runs), owner-signs a `kind:gateway` admission for this gateway + a `kind:console` admission for a fresh Console identity (MERGED into evie's existing enrollment, not clobbering other Gatewayes/revocations), writes evie's `evie-federation` Secret, restarts evie, then emits the provisioning blob to `~/android-dev/secrets/console-provisioning.json` (0600) and verifies the bridge with an authenticated probe. The Console imports that blob (paste/file) and is enrolled from it ALONE. The Gateway id is read authoritatively from the container's `GATEWAY_ID` env (run through `sanitizeGatewayId`), never from `docker logs`. Credential posture (owner-accepted deviation from the original "short-lived bootstrap token"): the blob is a durable, host-local 0600 file carrying the cluster SA token + the Console PRIVATE key. Re-running `--setup` is the rotation/recovery path; it de-admits the prior Console by snapshot replacement. For the untrusted-evie model, set `FEDERATION_OWNER_SIGN_PUB` to the owner key on each gateway; the script ABORTS with the exact remediation if the gateway pins a different owner than the root it just minted.
