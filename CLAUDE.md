...
- `src/` - Main source code
  ...
  - `shared/` - Shared utilities used by both arbiter and MCP
    ...
    - `evie-protocol.ts` - SELF-CONTAINED (zod-only) leaf owning the arbiter<->evie frame vocabulary, SYNCED verbatim into evie-bot (`cp` + SYNC-HASH): `EvieInboundFrameSchema` (tool_registry / tool_result / tool_error / loose phone_relay + loose switch_relay), `ToolCallFrameSchema`, `ChannelFileSchema` (re-exported by schemas.ts), and the federation routing envelope evie switches on - `SwitchRegisterParamsSchema`, `SwitchRelayRouteSchema` (opaque `payload`), `SwitchRelayReplyParamsSchema`, `FEDERATION_PROTOCOL_VERSION`. The register params also carry the optional admitted-identity proof (signPub/boxPub/admission-JSON/proof). Nothing imports into it
    ...
    - `enrollment.ts` - SYNCED leaf (imports `./admission.js` + `./crypto.js`): the typed QR payloads (`EnrollmentPayloadSchema`: enroll-owner/admit-switch/authorize-phone), the owner enroll ops (`EnrollOpSchema`: enroll_redeem/submit_admission/submit_revocation, codegen'd sealed), `EnrollResult`, `payloadSas`, `admissionFromScan`
    - `federation-protocol.ts` - switchboard-only INNER federation vocabulary evie never sees (content-blind): `FederatedOpSchema` (send/list_teams/wake/response_push), the `ReturnRouteSchema` reply-pin, the `SwitchRelayPayloadSchema` (always E2E-sealed: a single `sealed` envelope, no cleartext op - evie cannot read or forge it), and the full `SwitchRelayFrameSchema` the switch-relay pump validates. NOT codegen'd to Kotlin (cross-Switch is arbiter-to-arbiter; the phone reaches the mesh through its home Switch)
...
- **Trust model** (`shared/admission.ts`, SYNCED): `AdmissionSchema` (owner attests a subject's keys, `kind: switch|phone`), `SignedAdmission`, `Revocation`. Canonical signing bytes are a versioned newline-joined encoding (`ADMISSION_V1\n...`) reproduced byte-exact on node/Bun/Android - never sign raw JSON. `resolveAdmitted` returns the newest owner-verified, non-revoked admission. `DomainSnapshot` = the mirrored `{ownerSignPub, admissions, revocations}`.
...
- **Recovery** (clean-break, owner has 1 phone + 1 arbiter): delete the `evie-federation` Secret and each arbiter's `federation-allowlist.json` (keep `identity.json` so the arbiter re-presents its admit-switch QR), then re-enroll. There is no silent re-root: a snapshot / redeem for a different owner is refused.

The phone<->arbiter op path is now E2E sealed too (the symmetric twin of the cross-Switch seal): the phone seals each op into a `PhoneRelayFrame` (`{ opId, signerSignPub, sealed }`) to the arbiter's box key and signs with its enrolled key; the arbiter verifies the signer against an owner-signed `kind:phone` admission, decrypts, replay- and freshness-checks, and seals the reply back. evie stays content-blind (its `phone_relay` member is a loose passthrough). Only a pre-seal failure (malformed / unadmitted) returns a cleartext error so the phone can prompt enrollment. Still pending on the crypto track: the evie self-provisioning of the phone-bridge k8s objects (manual YAML for now), and the operational token retirement (`FEDERATION_REQUIRE_ADMISSION=true` + dropping `BRIDGE_TOKEN`/`ANDROID_BRIDGE_TOKEN`).
...
- **Host-agent:** the host orchestrator's `arbiter` channel identity is surfaced to the phone as `kind: "switch"` (shown first), reachable by `send` from the phone only (`channelOnly`). A send injects a `channel_push` into the orchestrator, which can dispatch to its devcontainers. The cli `host` wake-daemon stays hidden; container crosstalk to the host-agent is deferred to the federation phases.
...
- **Trust:** frames are zod-validated at the boundary (`PhoneRelayFrameSchema`) AND E2E sealed: the arbiter opens each frame through the `phoneSealer` (verifies the phone's signature against an owner-signed `kind:phone` admission, decrypts with its box key, replay- and freshness-checks) before dispatch, so it trusts a frame because it is signed by an admitted phone, not because it arrived on the evie WS. Each conversationId is bound to its first signing key (a phone cannot operate another install's mailbox by borrowing its conversationId). The bearer token is the coexistence-window relay gate at evie, retired by W5. Adds no new HTTP surface.
