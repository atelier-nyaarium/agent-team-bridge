...
#
#   ./provision-console.sh --setup    one-time host bootstrap: cluster cutover (console +
#                                      switch bridges) + root the Domain at the CONSOLE's
#                                      owner key + emit the Console's transport blob.
#   ./provision-console.sh --switch-transport  move the local Switch off port-forward onto
...
# transport-ONLY blob (cluster creds; no identity, no Switch keys). The Console generates
# its own identity, admits itself, and admits each Switch (by scanning the Switch's
# admit-switch QR) afterward. After setup, a menu offers the blob as a scannable QR or paste.
#
...
SWITCH_BRIDGE_YAML="../evie-bot/deploy/switch-bridge.yaml"
...
cutover() {
	note "cutover: applying console-bridge + switch-bridge objects (Services + SAs + Roles + tokens)"
	cat "$BRIDGE_YAML" | ki apply -f - >/dev/null || { err "kubectl apply $BRIDGE_YAML failed"; return 1; }
	cat "$SWITCH_BRIDGE_YAML" | ki apply -f - >/dev/null || { err "kubectl apply $SWITCH_BRIDGE_YAML failed"; return 1; }
...
# Switch afterward. The crypto is done by scripts/bootstrap-domain.ts (the real
# crypto.ts/admission.ts) so the rooted owner verifies byte-for-byte on the arbiter + app.
bootstrap_domain() {
	...
	if [ -n "$pin" ] && [ "$pin" != "$ownerPub" ]; then
		...
		err "  Set FEDERATION_OWNER_SIGN_PUB=$ownerPub on the arbiter (or unset it), restart it, then re-run --setup."
		return 1
	fi
	...
	# and admits this Switch (by scanning the Switch's admit-switch QR) - no host-side admit.
...
# Console generates its own owner + member identities and resolves Switch keys from the
# synced keyring, so the blob carries credentials only - no identity, no Switch keys.
emit_blob() {
	...
	# Also pack the switch-bridge creds so the Console can seal a bootstrap bundle for a
	# creds-less Switch it admits. Same SA-token-over-service-proxy shape; the forwarded
...
# Write the local Switch's service-proxy transport.json into its federation dir, so the
# arbiter reaches evie through the apiserver (off kubectl port-forward) on its next
# restart. The SA token + cluster CA come from the switch-bridge SA; the forwarded bridge
# token is the host's BRIDGE_TOKEN (the same value evie checks).
write_switch_transport() {
	local saToken caPem appToken apiUrl
	saToken=$(k get secret switch-bridge-proxy-token -o jsonpath='{.data.token}' 2>/dev/null | base64 -d)
	caPem=$(k get secret switch-bridge-proxy-token -o jsonpath='{.data.ca\.crt}' 2>/dev/null | base64 -d)
	[ -n "$saToken" ] && [ -n "$caPem" ] ||
		{ err "switch-bridge SA token not populated yet - re-run --setup in a few seconds"; return 1; }
	...
	SB_API="$apiUrl" SB_CA="$caPem" SB_SA="$saToken" SB_APP="$appToken" SB_NS="$NS" \
		...
		|| { err "writing switch transport.json failed"; return 1; }
	note "switch transport written: the arbiter uses the service-proxy after its next restart"
}
...
case "${1:-}" in
	--setup)
		require_container
		# write_switch_transport is intentionally NOT in this chain: it commits the local
		# Switch to the service-proxy WS, and if WS-over-proxy does not work on this cluster
		# the Switch could not connect (it only falls back to port-forward when transport.json
		# is absent). Validate the service-proxy path first, then run --switch-transport.
	...
	--qr)
		...
		;;
	--switch-transport)
		# Opt-in: move the local Switch off kubectl port-forward onto the service-proxy WS.
		# Run AFTER confirming WS-over-proxy works on this cluster; restart the arbiter to pick
		# up the transport.json. Reversible: delete /app/log/federation/transport.json to revert.
		require_container; write_switch_transport ;;
