...
import { MAX_BLOB_BYTES, ValueOpFrameSchema } from "../shared/router-protocol.js";
...
import { createWebSocketHandlers, resolveLiveIncarnation, type TeamRegistry, type WsData } from "./websocket.js";
...
export interface GatewayGraph {
	...
	close: () => Promise<void>;
	// Observation seams for the harness.
	...
	boot: () => BootState;
	federation: () => FederationSlice | null;
...
export function composeGateway(deps: GatewayDeps): GatewayGraph {
	...
	const ENROLL_TLS_PORT = config.enrollTlsPort;
	...
	const MAX_BLOB_STORE_BYTES = config.maxBlobStoreBytes;
	fs.mkdirSync(DATA_DIR, { recursive: true });

	const WAKE_TIMEOUT_MS = config.wakeTimeoutMs;
	...
	function enterArming(nonce: string): void {
		...
		const enrollLanHost = config.enrollLanHost;
		const enrollCert = deps.openEnrollTls ? generateEnrollCert(enrollLanHost) : null;
	...
	const wsHandlers = createWebSocketHandlers({
		...
		config: { HEARTBEAT_INTERVAL_MS, MISSED_PINGS_LIMIT, hostWsToken: config.hostWsToken },
		knownTeamPaths,
		offlineCatalog,
		hostSpawnPoints,
		wakeCoordinator,
		...
		const valueOp = (raw: unknown): void => {
			void (async () => {
				...
				const valueResult = composeValueResult({
					...
					seal: (plaintext, aad) => {
						const sealed = federation.contentKeyStore.seal(plaintext, {
							domainId,
							ownerSignPub,
							kind: aad.kind,
						});
				...
				const settled = await federation.routerClient.callTool("value_result", valueResult);
	...
	function startShareSweep(federation: FederationSlice): void {
		...
		shareSweepTimer = setInterval(() => {
			const dropped = federation.shareState.sweep(now(), THIRTY_DAYS_MS, isLive);
	...
	async function close(): Promise<void> {
		...
		clearInterval(tripwireTimer);
		clearInterval(persistTimer);
		clearInterval(presenceWatchTimer);
		clearInterval(awarenessTimer);
		clearInterval(wsHandlers.heartbeatInterval);
