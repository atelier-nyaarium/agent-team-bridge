...
- The Gateway's `cross_domain_share` value op writes the Router record and its own mirror in one operation, record first. The record is what admits the friend's rows. A refused record answers the value op as failed and leaves no mirror, and a mirror the Gateway's migration fence refuses withdraws the record and answers `migrating`, so the phone posts nothing itself.
